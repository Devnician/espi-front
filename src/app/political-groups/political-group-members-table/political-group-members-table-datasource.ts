...
/**
 * Data source for the PoliticalGroupMembersTable view. This class should
 * encapsulate all logic for fetching and manipulating the displayed data
 * (including sorting, pagination, and filtering).
 */
export class PoliticalGroupMembersTableDataSource extends DataSource<Political_Group_Members> {
  paginator: MatPaginator;
  counter: BehaviorSubject<number> = new BehaviorSubject(0);
  sort: MatSort;
  queryRef: QueryRef<GetPoliticalGroupMembersQuery>;
  loading: BehaviorSubject<any> = new BehaviorSubject(true);
  loading$ = this.loading.asObservable();
  currentPageData: Political_Group_Members[] = [];
  condition: BehaviorSubject<Political_Group_Members_Bool_Exp> =
    new BehaviorSubject({});
  constructor(
   ...
   */
  connect(): Observable<Political_Group_Members[] | any> {
    const limit: number = this.paginator.pageSize;
    const offset: number = this.paginator.pageIndex * this.paginator.pageSize;
    const order_by: Political_Group_Members_Order_By = {
      createdAt: Order_By.Asc,
    };

    // Combine everything that affects the rendered data into one update
    // stream for the data-table to consume.
    this.queryRef = this.politicalGroupsService.getPoliticalGroupMembers(
      limit,
      offset,
      {},
      order_by
    ...
    return merge(
      ...
      map(({ data, loading, errors }) => {
        ...
        this.counter.next(
          data.political_group_members_aggregate.aggregate.count
        );
        this.currentPageData =
          data.political_group_members as any as Political_Group_Members[];
        return data.political_group_members;
      })
    );
  }
